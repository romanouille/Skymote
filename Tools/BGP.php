...
for ($i = 1; $i <= 2; $i++) {
	...
	foreach ($file as $line) {
		...
		if ($ipVersion == 4) {
			$blockData = ReseauIo::cidrToRange($line[0]);
			...
			$blockStart = $block[0];
			if ($block[1] != 128) {
				$blockEnd = ReseauIo::long2ipv2(gmp_strval(gmp_sub(gmp_strval(gmp_add(ReseauIo::ip2longv2($blockStart), ReseauIo::cidrToHostsV6($block[1]))), 1)));
			} else {
				$blockEnd = $blockStart;
			}
		}
