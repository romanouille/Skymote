...
if (isset($_SERVER["PHP_AUTH_USER"]) && isset($_SERVER["PHP_AUTH_PW"]) && User::checkPassword($_SERVER["PHP_AUTH_USER"], $_SERVER["PHP_AUTH_PW"])) {
...
if ($dev) {
	if (isset($_COOKIE["admin"]) && $_COOKIE["admin"] == $config["admin"]["password"]) {
		$admin = true;
	} else {
		$admin = false;
	}
