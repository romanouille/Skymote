...
if (!empty($match) && isset($_SESSION["code"]) && isset($match[3]) && $_SESSION["code"] == $match[3]) {
    ...
    if (
        !in_array($protocol, ["tcp", "udp", "icmp"]) ||
        $port < 1 ||
        $port > 65535
    ) {
        http_response_code(400);
    ...
    if ($protocol != "icmp") {
        ...
        $pageDescription =
            "Le ping vers l'adresse IP $ip port $port ($protocol) a permis de recevoir $packetsCount paquet" .
            ($packetsCount > 1 ? "s" : "") .
            ".";
    } else {
        $pageTitle = "Ping vers l'adresse IP $ip ($protocol)";
        $pageDescription =
            "Le ping vers l'adresse IP $ip ($protocol) a permis de recevoir $packetsCount paquet" .
            ($packetsCount > 1 ? "s" : "") .
            ".";
    }
} else {
    $pageTitle = "Ping";
    $pageDescription =
		"Cet outil permet d'envoyer un ping vers une adresse IP avec un protocole spécifique ainsi qu'un port spécifique.";
}
